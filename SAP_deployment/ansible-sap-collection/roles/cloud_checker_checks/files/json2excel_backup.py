    ...
    def parse_args(self,argvars):
        try:
            ...
            if arg_len > 0:
                ...
                self.workbook_name = os.path.join(outpath, "Output.xlsx")
                #print("ARG3 ",args[4])
        ...
        try:
            ...
            for index, each in enumerate(self.lst_jsons):
                ...
                if (row-rowcheck) >= 5000:
                    ...
                    rowcheck=row
                    self.workbook.save(self.workbook_name.split(".xlsx")[0]+'_%s.xlsx'  %(filecnt+1))
                    filecnt +=1

            filewrite=self.workbook_name.split(".xlsx")[0]+'_%s.xlsx' %(filecnt+1)
...
try:
    ...
    filewrite = json_to_excel.workbook_name.split(".xlsx")[0]+'_%s.xlsx'  %(filecnt + 1)
