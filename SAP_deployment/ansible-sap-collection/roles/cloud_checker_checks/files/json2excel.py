...
class jsonToExcel():
    ...
    # merge_flatten
    # write_invalid_json
    # add_blank_columns_to_existing_table
    # flatten_many_jsons_and_merge
    # single_json_flatten_and_create_table
    # write_table_header
    # write_table_data
    # write_to_excel
    ...
    def parse_args(self,argvars):
        try:
            ...
            if arg_len > 0:
                ...
                outpath=argvars[1]
                #self.mini_check_args=argvars[3]
    ...
    def write_table_data(self,json_table,lstheader,row):
        ...
        for eachdat, eachval in json_table.items():
            if tot_jsonlen > 5000:
                countm +=1
                if countm == 500 * check:
                    #print("processing %s of total %s " % (countm, tot_jsonlen))
                    check = check + 1
    ...
    def write_to_excel(self,each_dir):
        ...
        try:
            ...
            # #COMBINE ALL JSON & WRITE TO EXCEL
            # Set start row and column number
            row = 1
            col = 1
            self.sheet = self.workbook.create_sheet(title=self.sheet_name)
            row = 1
            ...
            for index, each in enumerate(self.lst_jsons):
                ...
                if type(cpdata) == list :
                    ...
                    if type(lstdic) == dict or type(lstdic) == list:
                        ...
                        #For this iterate elements in list and flatten jsons seperately and combine these
                        if type(lstdic) == list:
                ...
                # self.log_file_name = self.filename + '_' + str(today) + '.log'
                #write table data
                row = self.write_table_data(json_table,lstheader,row)
                # if (row-rowcheck) >= 5000:
                #     #print("file name change, too large file ",self.workbook_name.split(".xlsx")[0]+'_%s.xlsx' %(filecnt+1))
                #     print("file name change, too large file ",
                #           self.workbook_name.split(".xlsx")[0] + '_%s.xlsx' % (today))
                #     rowcheck=row
                #     #self.workbook.save(self.workbook_name.split(".xlsx")[0]+'_%s_%s.xlsx'  %(today,filecnt+1))
                #     self.workbook.save(self.workbook_name.split(".xlsx")[0] + '_%s.xlsx' % (today))
                #     filecnt +=1

            #filewrite=self.workbook_name.split(".xlsx")[0]+'_%s_%s.xlsx' %(today,filecnt+1)
            # filewrite = self.workbook_name.split(".xlsx")[0] + '_%s.xlsx' % (today)

            #print("file write",filewrite)
    ...
    def write_json(self,data,wpath):
        try:
            #print("PATH TO CREATE HANA -",wpath)
    ...
    def write_hana_db_checks(self):
        try:
            ...
            #output='{"Output":"sample","Second":"sapme1"}'
...
try:
    ...
    filewrite = json_to_excel.workbook_name.split(".xlsx")[0]+'_%s.xlsx'  %(today)
    #filewrite = json_to_excel.workbook_name.split(".xlsx")[0] + '_%s_%s.xlsx' % (today, filecnt + 1)
    json_to_excel.workbook.save(filewrite)
except Exception as err:
    today = datetime.now()
    today = today.strftime('%Y%m%d_%I%M%S%p')
